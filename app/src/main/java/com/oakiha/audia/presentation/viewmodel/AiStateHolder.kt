package com.oakiha.audia.presentation.viewmodel

import android.content.Context
import com.oakiha.audia.R
import com.oakiha.audia.data.DailyMixManager
import com.oakiha.audia.data.ai.AiMetadataGenerator
import com.oakiha.audia.data.ai.AiBooklistGenerator
import com.oakiha.audia.data.ai.TrackMetadata
import com.oakiha.audia.data.preferences.UserPreferencesRepository
import com.oakiha.audia.data.model.Track
import dagger.hilt.android.qualifiers.ApplicationContext
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.asStateFlow
import kotlinx.coroutines.launch
import javax.inject.Inject
import javax.inject.Singleton

/**
 * Manages AI-powered features: AI Booklist Generation and AI Metadata Generation.
 * Extracted from PlayerViewModel.
 */
@Singleton
class AiStateHolder @Inject constructor(
    @ApplicationContext private val context: Context,
    private val aiBooklistGenerator: AiBooklistGenerator,
    private val aiMetadataGenerator: AiMetadataGenerator,
    private val dailyMixManager: DailyMixManager,
    private val userPreferencesRepository: UserPreferencesRepository,
    private val dailyMixStateHolder: DailyMixStateHolder
) {
    // State
    private val _showAiBooklistsheet = MutableStateFlow(false)
    val showAiBooklistsheet = _showAiBooklistsheet.asStateFlow()

    private val _isGeneratingAiBooklist = MutableStateFlow(false)
    val isGeneratingAiBooklist = _isGeneratingAiBooklist.asStateFlow()

    private val _aiError = MutableStateFlow<String?>(null)
    val aiError = _aiError.asStateFlow()

    private val _isGeneratingMetadata = MutableStateFlow(false)
    val isGeneratingMetadata = _isGeneratingMetadata.asStateFlow()

    private var scope: CoroutineScope? = null
    private var allTracksProvider: (() -> List<Track>)? = null
    private var favoriteTrackIdsProvider: (() -> Set<String>)? = null
    
    // Callbacks to interact with PlayerViewModel/UI
    private var toastEmitter: ((String) -> Unit)? = null
    private var playTracksCallback: ((List<Track>, Track, String) -> Unit)? = null // Tracks, startTrack, queueName
    private var openPlayerSheetCallback: (() -> Unit)? = null

    fun initialize(
        scope: CoroutineScope,
        allTracksProvider: () -> List<Track>,
        favoriteTrackIdsProvider: () -> Set<String>,
        toastEmitter: (String) -> Unit,
        playTracksCallback: (List<Track>, Track, String) -> Unit,
        openPlayerSheetCallback: () -> Unit
    ) {
        this.scope = scope
        this.allTracksProvider = allTracksProvider
        this.favoriteTrackIdsProvider = favoriteTrackIdsProvider
        this.toastEmitter = toastEmitter
        this.playTracksCallback = playTracksCallback
        this.openPlayerSheetCallback = openPlayerSheetCallback
    }

    fun showAiBooklistsheet() {
        _showAiBooklistsheet.value = true
    }

    fun dismissAiBooklistsheet() {
        _showAiBooklistsheet.value = false
        _aiError.value = null
        _isGeneratingAiBooklist.value = false
    }

    fun generateAiBooklist(prompt: String, minLength: Int, maxLength: Int, saveAsBooklist: Boolean = false) {
        val scope = this.scope ?: return
        val allTracks = allTracksProvider?.invoke() ?: emptyList()
        val favoriteIds = favoriteTrackIdsProvider?.invoke() ?: emptySet()

        scope.launch {
            _isGeneratingAiBooklist.value = true
            _aiError.value = null

            try {
                // Generate candidate pool using DailyMixManager logic
                val candidatePool = dailyMixManager.generateDailyMix(
                    allTracks = allTracks,
                    favoriteTrackIds = favoriteIds,
                    limit = 120
                )

                val result = aiBooklistGenerator.generate(
                    userPrompt = prompt,
                    allTracks = allTracks,
                    minLength = minLength,
                    maxLength = maxLength,
                    candidateTracks = candidatePool
                )

                result.onSuccess { generatedTracks ->
                    if (generatedTracks.isNotEmpty()) {
                        if (saveAsBooklist) {
                            val BooklistName = "AI: ${prompt.take(25)}${if (prompt.length > 25) "..." else ""}"
                            val TrackIds = generatedTracks.map { it.id }
                            userPreferencesRepository.createBooklist(
                                name = BooklistName,
                                TrackIds = TrackIds,
                                isAiGenerated = true
                            )
                            toastEmitter?.invoke("AI Booklist '$BooklistName' created!")
                            dismissAiBooklistsheet()
                        } else {
                            // Play immediately logic
                            dailyMixStateHolder.setDailyMixTracks(generatedTracks)
                            playTracksCallback?.invoke(generatedTracks, generatedTracks.first(), "AI: $prompt")
                            openPlayerSheetCallback?.invoke()
                            dismissAiBooklistsheet()
                        }
                    } else {
                        _aiError.value = context.getString(R.string.ai_no_Tracks_found)
                    }
                }.onFailure { error ->
                    _aiError.value = if (error.message?.contains("API Key") == true) {
                        context.getString(R.string.ai_error_api_key)
                    } else {
                        context.getString(R.string.ai_error_generic, error.message ?: "")
                    }
                }
            } finally {
                _isGeneratingAiBooklist.value = false
            }
        }
    }

    fun regenerateDailyMixWithPrompt(prompt: String) {
        val scope = this.scope ?: return
        val allTracks = allTracksProvider?.invoke() ?: emptyList()
        val favoriteIds = favoriteTrackIdsProvider?.invoke() ?: emptySet()
        val currentDailyMixTracks = dailyMixStateHolder.dailyMixTracks.value

        scope.launch {
            if (prompt.isBlank()) {
                toastEmitter?.invoke(context.getString(R.string.ai_prompt_empty))
                return@launch
            }

            _isGeneratingAiBooklist.value = true
            _aiError.value = null

            try {
                val desiredSize = currentDailyMixTracks.size.takeIf { it > 0 } ?: 25
                val minLength = (desiredSize * 0.6).toInt().coerceAtLeast(10)
                val maxLength = desiredSize.coerceAtLeast(20)
                
                val candidatePool = dailyMixManager.generateDailyMix(
                    allTracks = allTracks,
                    favoriteTrackIds = favoriteIds,
                    limit = 100
                )

                val result = aiBooklistGenerator.generate(
                    userPrompt = prompt,
                    allTracks = allTracks,
                    minLength = minLength,
                    maxLength = maxLength,
                    candidateTracks = candidatePool
                )

                result.onSuccess { generatedTracks ->
                    if (generatedTracks.isNotEmpty()) {
                        dailyMixStateHolder.setDailyMixTracks(generatedTracks)
                        toastEmitter?.invoke(context.getString(R.string.ai_daily_mix_updated))
                    } else {
                        toastEmitter?.invoke(context.getString(R.string.ai_no_Tracks_for_mix))
                    }
                }.onFailure { error ->
                    _aiError.value = error.message
                    toastEmitter?.invoke(context.getString(R.string.could_not_update, error.message ?: ""))
                }
            } finally {
                _isGeneratingAiBooklist.value = false
            }
        }
    }

    suspend fun generateAiMetadata(Track: Track, fields: List<String>): Result<TrackMetadata> {
        _isGeneratingMetadata.value = true
        return try {
            aiMetadataGenerator.generate(Track, fields)
        } finally {
            _isGeneratingMetadata.value = false
        }
    }

    fun onCleared() {
        scope = null
    }
}
